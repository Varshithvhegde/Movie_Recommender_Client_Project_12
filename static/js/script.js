  ...
  if (!(upBtn == null)) {
    var handleClick = function (event) {
      ...
      if (!phoneRegex.test(mobile)) {
        var errorMsg = "Please enter a 10-digit phone number.";
        document.getElementById("error").innerHTML = errorMsg;
        return;
      }
      ...
      if (!passwordRegex.test(password)) {
        var errorMsg =
          "Please enter a password with at least 8 characters, one lowercase letter, one uppercase letter, and one number.";
        document.getElementById("error").innerHTML = errorMsg;
        return;
      }
  ...
  $.post(
    ...
    function (data, status) {
      ...
      for (var i = 0; i < data[0].genre_movies.length; i++) {
        htmlContent =
          htmlContent +
          "<div class='col-6 col-sm-4 col-md-2 movie-card'><a href='/movie/" +
          data[0].genre_movies[i] +
          "'><img src='" +
          data[1].genre_posters[i] +
          "' alt='' style='width: 100%;height: auto;'><p>" +
          data[0].genre_movies[i] +
          "</p></a></div>";
      }
    ...
    function (data, status) {
      ...
      for (var i = 0; i < data[0].year_movies.length; i++) {
        htmlContent =
          htmlContent +
          "<div class='col-6 col-sm-4 col-md-2 movie-card'><a href='/movie/" +
          data[0].year_movies[i] +
          "'><img src='" +
          data[1].year_posters[i] +
          "' alt='' style='width: 100%;height: auto;'><p>" +
          data[0].year_movies[i] +
          "</p></a></div>";
      }
